...
if __name__ == '__main__':
    try:
        sub = rospy.Subscriber("/mangu/odom", Odometry, newOdom)
        pub = rospy.Publisher('/mangu/cmd_vel_mux/input/navi', Twist, queue_size=10)
        rospy.init_node('mangu_move', anonymous=True)

        rate = rospy.Rate(10)
        vel_command = Twist()
        #fname = '/home/abhishek/Documents/nodes_optimal_part4.txt'
        gname = '/home/abhishek/Documents/RRT/final_path2.txt'
        ...
        for p in gemp:
            ...
            while(True):
                ...
                if abs(angle - yaw)> 0.05:
                    ...
                    rate.sleep()
                elif abs(math.sqrt(((goal_x-(x+0.91)) ** 2) + ((goal_y-(y+3.0)) ** 2)))>0.108:
                    print(abs(math.sqrt(((goal_x-(x+0.91)) ** 2) + ((goal_y-(y+3.0)) ** 2))),'distance')
                    vel_command.linear.x = 0.1
                    vel_command.angular.z = 0.0
                    pub.publish(vel_command)
                    #print(abs(math.sqrt(((goal_x-(x+3.5)) ** 2) + ((goal_y-(y+3.0)) ** 2))))
                    print 'x=', (x+0.91), 'y=',(y+3.0), 'moving'
                    #rospy.loginfo(vel_command)
                    rate.sleep()
                else:
